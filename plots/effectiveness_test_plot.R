...
createVirtualInstance <- function(faster_algo, total_time_faster_algo, total_time_slower_algo) {
  write.csv(faster_algo, file = "faster_algo.csv", quote = F, row.names = F)
  virtual_instances_faster_algo <- data.frame()
  algo <- faster_algo
  algo <- droplevels(algo)
  current_sample <- sample_n(algo, 1)
  current_time <- current_sample$Time[[1]]
    ...
    for ( Graph in levels(factor(algo_1$Graph)) ) {
      ...
      for ( K in levels(factor(algo_1$K)) ) {
        ...
        if ( total_time_algo_1 < total_time_algo_2 ) {
          sum_total_time_algo_1 <- sum(instance_algo_1$Time)
          ...
          sum_total_time_algo_2 <- sum(instance_algo_2$Time)
        ...
        virtual_instances_algo_2$NumPEs <- 1
        virtual_instances_algo_1 <- aggregate_data(virtual_instances_algo_1)
        virtual_instances_algo_2 <- aggregate_data(virtual_instances_algo_2)
...
create_effectiveness_test <- function(effectiveness_test_df, algo_name_1, algo_name_2, objective) {
...
quality <- create_performance_profile(virtual_algo_1, virtual_algo_2, column.objective = objective)
return(quality)
}
